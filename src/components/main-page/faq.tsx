...
export function Faq() {
    return (
        <section id="faq" className="container mx-auto max-w-6xl py-24 md:py-0 md:mb-24">
            <div className="grid grid-cols-1 gap-12 md:grid-cols-7">
                <div className="md:col-span-3">
                    <p className="ml-0.5 font-medium uppercase text-emerald-700">FAQ</p>
                    ...
                    <Accordion type="single" collapsible className="w-full">
                        {faqData.map((faq, index) => (
                            <AccordionItem value={`item-${index + 1}`} key={index} className="group border-b">
                                <AccordionTrigger className="py-6 text-left text-lg font-medium hover:no-underline hover:text-emerald-600 transition-colors">
                                    {faq.question}
                                </AccordionTrigger>
