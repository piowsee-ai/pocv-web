...
export function WelcomeStats() {
    ...
    const stats = [
        ...
        },
        {
            icon: Clock,
            label: "Last Update",
            value: isPending || isError ? "-" : (data?.data?.[0]?.updatedAt 
                ? new Date(data.data[0].updatedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })
                : "N/A"),
        ...
        return (
            <section className="container mx-auto max-w-7xl px-4 md:px-6 py-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[1, 2, 3, 4].map((i) => (
                        <div key={i} className="rounded-xl bg-gray-100 dark:bg-gray-800 p-4 animate-pulse">
        ...
        return (
            <section className="container mx-auto max-w-7xl px-4 md:px-6 py-8">
                ...
                <div className="rounded-xl border border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-800 p-6 text-center">
                    <p className="text-red-600 dark:text-red-400 mb-3">Gagal memuat data. Silakan coba lagi.</p>
                    <button 
                        onClick={() => refetch()} 
                ...
                {stats.map((stat, index) => (
                    ...
                    >
                        ...
                        <div className="mt-3">
                            ...
                            <p className={`font-bold ${stat.isText ? 'text-xs mt-1' : 'text-2xl'} text-foreground`}>
                                {stat.value}
                            </p>
                        </div>
                        <div className={`absolute -right-4 -bottom-4 h-20 w-20 rounded-full bg-gradient-to-br ${stat.color} opacity-10 blur-xl`} />
                    </div>
                ))}
            </div>
        </section>
